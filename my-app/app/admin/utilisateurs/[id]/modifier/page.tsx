...
export default function ModifierUtilisateurPlateformePage() {
  ...
  const charger = useCallback(() => {
    ...
    fetch(`/api/admin/utilisateurs/${id}`)
      ...
      .then((data: UtilisateurDetail) => {
        setUtilisateur(data)
        setFormInfos({ nom: data.nom, prenom: data.prenom, email: data.email ?? '', role: data.role, actif: data.actif })
      })
      .catch(() => {
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">{utilisateur.prenom} {utilisateur.nom} — {utilisateur.paroisse.nom}</p>
      </div>
      ...
      <form onSubmit={soumettreInfos} noValidate>
        <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={utilisateur.paroisse.nom}
              disabled
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-500 bg-gray-50 cursor-not-allowed"
            />
            <p className="mt-1 text-xs text-gray-400">La paroisse ne peut pas être modifiée depuis cette page.</p>
          </div>
