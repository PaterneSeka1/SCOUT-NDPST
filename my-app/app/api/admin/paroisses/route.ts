...
export async function POST(request: NextRequest) {
  try {
    ...
    if (MONO_PAROISSE) {
      const nbParoisses = await prisma.paroisse.count()
      if (nbParoisses >= 1) {
        return NextResponse.json(
          { erreur: 'Cette version ne prend en charge qu’une seule paroisse. La création d’une paroisse supplémentaire est désactivée.' },
          { status: 400 },
        )
      }
    }
